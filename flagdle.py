...
def menu_principal():
    # same as the flagdle menu, but you choose between "Flagdle" and "Countrydle"
    flagdle_color = unactiv_color
    countrydle_color = unactiv_color
    flagdle_button = pygame.Rect(SCREEN_WIDTH/2 - 100, SCREEN_HEIGHT/2 - button_vertical_sepraration/2, 200, 50)
    countrydle_button = pygame.Rect(SCREEN_WIDTH/2 - 100, SCREEN_HEIGHT/2 + button_vertical_sepraration/2, 200, 50)
    choosed = False
    while not choosed:
        flagdle_button_text = base_font.render("Flagdle", True, (255, 255, 255))
        countrydle_button_text = base_font.render("Countrydle", True, (255, 255, 255))
        # align each text to the center of each buttons (use button position and size)
        screen.fill(BLACK)
        pygame.draw.rect(screen, flagdle_color, flagdle_button, border_radius=border_radius)
        pygame.draw.rect(screen, countrydle_color, countrydle_button, border_radius=border_radius)
        screen.blit(flagdle_button_text, (flagdle_button.x + flagdle_button.width/2 - flagdle_button_text.get_width()/2, flagdle_button.y + flagdle_button.height/2 - flagdle_button_text.get_height()/2))
        screen.blit(countrydle_button_text, (countrydle_button.x + countrydle_button.width/2 - countrydle_button_text.get_width()/2, countrydle_button.y + countrydle_button.height/2 - countrydle_button_text.get_height()/2))
        pygame.display.flip()
        for event in pygame.event.get():
            if flagdle_button.collidepoint(pygame.mouse.get_pos()):
                flagdle_color = activ_color
            else:
                flagdle_color = unactiv_color

            if countrydle_button.collidepoint(pygame.mouse.get_pos()):
                countrydle_color = activ_color
            else:
                countrydle_color = unactiv_color
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if flagdle_button.collidepoint(event.pos):
                    flagdle_button = pygame.Rect(SCREEN_WIDTH/2 - 175/2, SCREEN_HEIGHT/2 - button_vertical_sepraration/2+2.5, 175, 45)
                                        
                if countrydle_button.collidepoint(event.pos):
                    countrydle_button = pygame.Rect(SCREEN_WIDTH/2 - 175/2, SCREEN_HEIGHT/2 + button_vertical_sepraration/2+2.5, 175, 45)
...
def flagdle():
    global LIVES, lives, SCORE, LIVES_POS, SCORE_POS, GUESS_POS, TEXT_WIDTH, border_radius, base_font, big_font, user_text, WIDTH, HEIGHT, input_rect, unactiv_color, activ_color, active, guess, FLAGS_PATH
    flags = []
...
def countrydle():
    # same as flagdle, but the path is different, it's country/ and inside are .png of the country shape
    # main
    return False
